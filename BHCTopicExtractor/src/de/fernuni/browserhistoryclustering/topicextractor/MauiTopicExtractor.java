...
public class MauiTopicExtractor implements OptionHandler {
   ...
   Config v_Config = Config.getInstance();
	...
	public Stopwords stopwords = new StopwordsEnglish(
	      v_Config.getBaseDir() + v_Config.getDataDir() + v_Config.getMauiStopwordsFile());
	...
	public void extractKeyphrases(HashSet<String> fileNames) throws Exception {
		...
		} else if (wikipediaServer.equals("localhost")
		...
		for (String fileName : fileNames) {
					...
					if (printer != null) {
						...
						if (printGraph) {
							...
							if (article != null) {
								...
								if (debugMode) {
									System.err
											.println("Couldn't find article for "
													+ topic
													+ " in "
													+ documentTopicsFile);
								}
						...
						if (additionalInfo) {
							...
							printer.print(topRankedInstances[i]
									.stringValue(mauiFilter
											.getNormalizedFormIndex()));
							printer.print("\t");
							printer.print(Utils.doubleToString(
									topRankedInstances[i].value(mauiFilter
			...
			if (printGraph) {
				String graphFile = documentTopicsFile.getAbsolutePath()
						.replace(".key", ".gv");
			...
			if (numExtracted > 0) {
				...
				precisionStatistics.addElement(new Double(numCorrect
						/ numExtracted));
				recallStatistics.addElement(new Double(numCorrect
						/ totalCorrect));
	...
	public ArrayList<Instance> extractKeyphrases(String p_TextId, String p_Text)
		...
		// Check whether there is actually any data
		if (p_Text.length() == 0 || p_Text == null) {
		...
		ArrayList<Instance> v_Instances = new ArrayList<Instance>();
	 ...
	 * topics of the document. The nodes are the topics and the edges are
	 * relations between them as computed using the Wikipedia Miner. Only
	 * possible if Wikipedia data is provided.
	 * 
	 * @param topics
	 * @param root
	 * @param outputFile
	 */
	public void computeGraph(HashMap<Article, Integer> topics, String root,
			String outputFile) {
		...
		try {
			...
			if (debugMode) {
				System.err.println("Printing graph information into "
						+ outputFile);
			}
