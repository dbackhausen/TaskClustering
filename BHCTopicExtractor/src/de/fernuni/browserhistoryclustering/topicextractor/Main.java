...
import de.fernuni.browserhistoryclustering.common.config.StringDefaults;
	   ...
	   Extractor v_TopicExtractor = new Extractor();
		...
		for (File v_File : v_TextFiles) {
			...
			Map<String, Double> v_Normtopic2Value = v_TopicExtractor
					.extractNormalizedKeyphrase(v_Text, v_File.getName(), v_Stem2Full);
			
			v_Filename2Normtopic2Value.put(v_File.getName(), v_Normtopic2Value);
			v_Filename2Normtopic2Topic.put(v_File.getName(), v_Stem2Full);
		}
		
		Normtopic2Topic v_Normtopic2Topic = new Normtopic2Topic(v_Filename2Normtopic2Topic);
		v_Normtopic2Topic.save(v_Config.getBaseDir() +
		      v_Config.getDataDir() + v_Config.getNormtopic2TopicFile());
		      
		
		NormtopicValues v_NormtopicValues = new NormtopicValues(v_Filename2Normtopic2Value);
		v_NormtopicValues.save(v_Config.getBaseDir() +
            v_Config.getDataDir() + v_Config.getNormtopic2ValueFile());
